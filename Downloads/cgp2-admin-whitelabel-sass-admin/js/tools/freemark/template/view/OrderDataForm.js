Ext.define('CGP.tools.freemark.template.view.OrderDataForm', {
    ...
    initComponent: function () {
        var me = this;
        me.attributeStore = Ext.data.StoreManager.get('contentAttribute')
        me.items = [
            {
                xtype: 'displayfield',
    ...
    listeners: {
        afterrender: function (comp) {
            if (comp.attributeStore) {
                comp.attributeStore.each(function (item) {
                    var attribute = item.raw['attribute'];
            ...
            if (item.itemId=='attributeContainer') {
                item.items.items.forEach(function (el) {
                    productAttributeValueMap[el.attribute.id]={
                        ...
                        "attributeOptionIds" : el.getValue()
                    }
                })
