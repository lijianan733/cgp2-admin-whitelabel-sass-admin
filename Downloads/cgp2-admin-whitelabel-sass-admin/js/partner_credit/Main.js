...
Ext.onReady(function () {
    ...
    var page = Ext.create('Ext.ux.ui.GridPage', {
        i18nblock: 'partner信贷管理',
        block: 'partner_credit',
        editPage: 'edit.html',
        tbarCfg: {
            btnCreate: {
                handler: function (btn) {
                    controller.showCreateCreditWin();
                }
            },
            btnDelete: {
                disabled: true,
        ...
        gridCfg: {
            ...
            columns: [
                            ...
                            items: [
                                {
                                    xtype: 'button',
                                    text: '修改',
                                    hidden: status == 'Invalid',
                                    handler: function (btn) {
                                        JSOpen({
                                            id: 'partner_credit_edit',
                                            url: path + `partials/partner_credit/edit.html?partnerId=${partner.id}&id=${id}&action=edit&status=${status}`,
                                            title: i18n.getKey('修改') + '_' + `partner信贷配置(${id})`,
                                            refresh: true
                                        });
                                    }
                                },
                                {
                                    ...
                                    handler: function (btn) {
                                        JSOpen({
                                            id: 'partner_credit_edit',
                                            url: path + `partials/partner_credit/edit.html?partnerId=${partner.id}&id=${id}&action=read&status=${status}`,
                                            title: i18n.getKey('查看') + '_' + `partner信贷配置(${id})`,
                                            refresh: true
                                        });
                                    }
                                },
                ...
                },
                {
                    ...
                    sortable: false,
                    renderer: function (value) {
                        return JSCreateFont('orange', true, value)
                    }
                },
                {
                    text: '管理信贷额',
