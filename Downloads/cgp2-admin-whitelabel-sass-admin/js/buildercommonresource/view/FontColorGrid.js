...
Ext.define('CGP.buildercommonresource.view.FontColorGrid', {
    extend: 'CGP.common.commoncomp.QueryGrid',
    alias: 'widget.fontcolorgrid',
    controller: null,
...
*/
        ...
        me.gridCfg = {
            store: Ext.create('CGP.buildercommonresource.store.CommonFontColorStore', {}),
            frame: false,
            editAction: false,
            deleteActionHandler: function (gridview, rowIndex, colIndex, view, event, record, dom) {
                ...
                Ext.Msg.confirm(i18n.getKey('prompt'), i18n.getKey('deleteConfirm'), function (selector) {
                    if (selector == 'yes') {
                        queryGrid.controller.operateResource('fontColor', 'DELETE', id, grid);
                    }
                })
            },
            tbar: {
                            ...
                            items: [
                                {
                                    ...
                                    gridCfg: {
                                        store: Ext.create('CGP.buildercommonresource.store.CommonFontColorStore', {
                                            proxy: {
                                                type: 'uxrest',
                                                url: adminPath + 'api/commonbuilderresourceconfigs/V2/remain/fontColors',
                                                reader: {
                                                    type: 'json',
                                                    root: 'data.content'
                                                }
                            ...
                            bbar: [
                                '->',
                                {
                                    ...
                                    handler: function (btn) {
                                        ...
                                        if (ids.length > 0) {
                                            queryGrid.controller.operateResource('fontColor', 'POST', ids, queryGrid.grid);
                                            win.close();
                                        }
                                    }
                                },
                ...
                btnDelete: {
                    handler: function (btn) {
                        ...
                        if (ids.length > 0) {
                            Ext.Msg.confirm(i18n.getKey('prompt'), i18n.getKey('deleteConfirm'), function (selector) {
                                if (selector == 'yes') {
                                    queryGrid.controller.operateResource('fontColor', 'DELETE', ids, grid);
                                    grid.store.load();
                                }
                            })
                        }
