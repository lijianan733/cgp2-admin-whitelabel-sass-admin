 ...
 *
 */
Ext.define('CGP.common.productsearch.filter.Panel', {
    extend: 'Ext.form.Panel',
    alias: 'widget.uxproductfilter',
    mixins: {
    ...
    constructor: function (config) {
        ...
        /*
         config.filterData = config.filterData || {};
         */
        /*if (config.websiteId == null) {
...
//修改接口后，无需传该参数了，传入隐藏的[{excludePartner.id","value":146,"type":"number"}]
        ...
        /*
         Ext.applyIf(config.filterData,{
         xtype : 'textfield',
         name : 'excludeIds',
         hidden : true,
         value : function(){
         if(Ext.isEmpty(config.filterDate)){
         return ;
         }else if(Ext.isString(config.filterDate)){
         return config.filterDate;
         }else{
         var value = [];
         for(var i = 0 ; i < config.filterDate.length;i++){
         value.push( config.filterDate[i].get("id"));
         }
         return value.join(",");
         }
         }()
