 ...
 */
Ext.define('CGP.common.valueEx.controller.Controller', {
    ...
    editOrAddItems: function (view, rowIndex, configurableId) {
        ...
        var alertwindowFormContainer = Ext.create('CGP.common.valueEx.view.ValidExpressionContainer', {
            ...
            items: [
                {
                    ...
                    store: Ext.create('Ext.data.Store', {
                        ...
                        data: [
                            ...
                            {clazz: 'ProductAttributeValueEx', value: 'com.qpp.cgp.domain.product.value.ProductAttributeValueEx'}
                        ]
                    }),
                    displayField: 'clazz',
                    valueField: 'value',
                    listeners: {
                        'change': function (view, newValue, oldValue) {
                            for (var i = 2; i < view.up().items.items.length; i++) {
                                view.up().remove(view.up().items.items[i]);
                            }
                            var addItme = {};
                            switch (newValue) {
    ...
    selectConstraintType: function (skuAttributeId, recordId, tab, createOrEdit, inputTypeClazz, data, configurableId, record) {
