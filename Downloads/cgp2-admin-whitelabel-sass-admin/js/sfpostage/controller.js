Ext.ns('Qpp.CGP.StandardPostage.Controller');
...
Qpp.CGP.StandardPostage.Controller.expandBody = function (rowNode, record, expandRow) {

    var id = record.get('id');
    var gridId = 'standard-postage-rule-grid-' + id;
    var containerId = 'standard-postage-rule-content-' + id;

    //    if (!Ext.getCmp(gridId)) {
    if (Ext.getCmp(gridId))
        Ext.getCmp(gridId).destroy();

    Qpp.CGP.StandardPostage.Controller._generateRuleGrid(record, gridId, containerId);


}


Qpp.CGP.StandardPostage.Controller._generateRuleGrid = function (record, gridId, containerId) {
    var controller = Qpp.CGP.StandardPostage.Controller;
    ...
    var grid = new Ext.grid.Panel({
        ...
        columns: [{
            ...
            text: i18n.getKey('amountPromotion')
        }, {
            dataIndex: 'promotion',
...
Qpp.CGP.StandardPostage.Controller.addPostageRule = function () {
...
Qpp.CGP.StandardPostage.Controller.savePostageRule = function () {
    var ruleStore = this.ownerCt.ownerCt.getStore();
    var postageStore = Ext.data.StoreManager.lookup('standardPostageStore');
    ...
    function savePostage(data) {
        ...
        var request = {
            url: adminPath + 'api/admin/postage/standard/' + data.id + '?access_token=' + Ext.util.Cookies.get('token'),
            method: 'PUT',
            jsonData: data,
