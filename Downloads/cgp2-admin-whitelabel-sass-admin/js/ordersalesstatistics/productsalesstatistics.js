...
Ext.onReady(function () {
    ...
    //这是图表chart的Store数据
    var store = Ext.data.StoreManager.lookup('MonthlyProductStatisticsStore');
    // js的时间日期格式化函数
...
//                }
                ...
                listeners: {
                    select: function (combo, records) {
                        ...
                        if (record.get('id') != null) {
                            mainCategory.websiteSelector.setValue(record.get('id'));
                ...
                listeners: {
                    expand: function (field) {
                        var website = field.ownerCt.getComponent('website');
                        var value = website.getValue();
                        if (!Ext.isNumber(value)) {
                            field.collapse();
                            Ext.Msg.alert('Info', 'Select website first!');
                        }
                    }
                }
            }, {
...
//				},
                ...
                tips: {
                    ...
                    renderer: function (storeItem, item) {
                        this.setTitle(storeItem.get('money') + ': ' + storeItem.get('product') + ' $');
                    }
                }
            }]
        }, {
            ...
            series: [{
                ...
                tips: {
                    ...
                    renderer: function (storeItem) {
                        ...
                        this.setTitle(storeItem.get('product') + '<br> $' + storeItem.get('money') + '<br>'
    ...
    function loadSaleStore() {
        ...
        var mainCategory = orderSalesPage.getComponent('filter').getComponent('mainCategory').getValue();

        if (date != null) {
            panel.getComponent('chartcolumn').store.load({

                params: {month: date, statusId: status, websiteId: website, mainCategoryId: mainCategory}
//				params:{month: date}
            });
        }
    };
