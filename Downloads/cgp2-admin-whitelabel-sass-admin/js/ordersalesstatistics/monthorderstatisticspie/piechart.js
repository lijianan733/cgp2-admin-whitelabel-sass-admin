...
statistics.columnChart.createPieChart = function(firstYear,status){
	...
	var piePage = Ext.create("Ext.panel.Panel",{
		...
		bbar: [{
            text: i18n.getKey('previousmonth'),  //'上一月',
...
//                },
     ...
     var t = new Ext.Template([
	    '<div >', //style="line-height:50px;"
	        '<span >'+i18n.getKey('time')+'：{first} &nbsp&nbsp '+i18n.getKey('orderStatus')+' : {status}</span>',
	    '</div>'
	]);
	var introductText = new Ext.Template([
		'<div>',
			'<span> {first}年每月销售额每个网站占据比例</span>',
		'<div>'
	]);
	...
	var htmlstr = t.apply({first: firstYear,   status: statusStr});
