Ext.onReady(function () {
    ...
    var store = Ext.create('CGP.partner.serviceomt.store.ServiceOmtStore', {
        type: 'backstage',
        partnerId: mailTemplateTabs.partnerId
    });
    var OrderStatuses = parent.Ext.StoreManager.get('allOrderStatus');
...
//				disabled : true,
                handler: function (view) {
                    mailTemplateTabs.addServerEditTab(null, i18n.getKey('server') + i18n.getKey('notifyEmailConfig'));
                }
            },
            btnDelete: {
                handler: function (view) {
                    ...
                    items: [
                        {
                            ...
                            handler: function (gridview, recordIndex, cellIndex, fun, button, record) {
                                var id = record.getId();
                                mailTemplateTabs.addServerEditTab(id, i18n.getKey('server') + i18n.getKey('notifyEmailConfig'));
                            }
                        },
                        {
        ...
        filterCfg: {
            items: [
                ...
                },
                {
                    ...
                    xtype: 'numberfield',
                    hidden: true,
                    value: parseInt(mailTemplateTabs.partnerId)
                }
            ],
            listeners: {
                afterrender: function (combo) {
