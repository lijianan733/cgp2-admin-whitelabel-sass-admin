 ...
 */
Ext.define('CGP.partner.view.orderstatuschangenotifyconfig.controller.Controller', {
    /**
    ...
    editRestHttpRequestConfig: function (createOrEdit, store, tab, record) {
        if (createOrEdit == 'create') {
            record = Ext.create('CGP.partner.view.orderstatuschangenotifyconfig.model.RestHttpRequestConfigModel');
        }
        ...
        editOrCreateRestRequestConfigPanel = Ext.create('CGP.partner.view.orderstatuschangenotifyconfig.view.EditOrCreateRestRequestConfig', {
    ...
    showAllEnablePlaceholder: function () {
        var win = Ext.widget('showjsondatawindow', {
            rawData: CGP.partner.view.orderstatuschangenotifyconfig.config.Config.data,
            title: '查看可用占位符'
        });
        win.show();
    },
    ...
    saveFormValue: function (form, partnerId, createOrEdit, recordId, recordData) {
        ...
        var method = 'PUT';
        var url = adminPath + 'api/partners/' + partnerId + '/orderStatusChangeNotifyConfigs/' + recordId;
        if (createOrEdit == 'create') {
            method = 'POST'
            ...
            success: function (response) {
                ...
                if (responseMessage.success) {
                    Ext.Msg.alert(i18n.getKey('prompt'), i18n.getKey('save') + i18n.getKey('success'), function () {
                        ...
                        var orderStatusChangeEmailNotifyConfigFrame = top.Ext.getCmp("tabs").getComponent('orderstatuschangenotifyconfigedit');
                        ...
                        var orderstatuschangenotifyconfigPanel = tabs.getComponent('orderstatuschangenotifyconfig');
                        tabs.setActiveTab(orderstatuschangenotifyconfigPanel);
                        top.document.getElementById("tabs_iframe_orderstatuschangenotifyconfig").contentWindow.Ext.getCmp('orderstatuschangenotifyconfigGrid').store.load();
                        myMask.hide();
                    });
                } else {
    ...
    editNotifyEmailConfig: function (createOrEdit, store, tab, record, type) {
        if (createOrEdit == 'create') {
            record = Ext.create('CGP.partner.view.orderstatuschangenotifyconfig.model.NotifyEmailConfigModel');
        }
        var editOrCreateEmailConfigPanel = tab.getComponent('editOrCreateEmailConfigPanel2');
        tab.remove(editOrCreateEmailConfigPanel);
        editOrCreateEmailConfigPanel = Ext.create('CGP.partner.view.orderstatuschangenotifyconfig.view.EditOrCreateNotifyEmailConfig', {
