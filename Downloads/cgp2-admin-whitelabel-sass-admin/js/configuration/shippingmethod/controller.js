    ...
    configPaymentStore.load({
        ...
        callback: function (records, options, success) {
            if ((records == null || records[0] == null) && isEdit == false) {
                Ext.Msg.alert(i18n.getKey('prompt'), i18n.getKey('noconfigShipping') + '!', function (btn) {
                    // newWindow.close();
                });
            } else {
                if (isEdit == false) {
...
//		renderTo : Ext.getCmp("viewport").el.dom,
            ...
            items: [{
                ...
                items: [{
                    ...
                    allowBlank: false,
                    name: 'website',
                    value: Number(getQueryString("websiteId")) || 11,
                }],
                buttons: [{
                    text: i18n.getKey('multilingual') + i18n.getKey('config'),
                    hidden: !isEdit,
                    handler: function (button) {
                        var id = newWindow.shippingId;
                        var title = 'shippingMethod';
                        var multilingualKey = 'com.qpp.cgp.domain.common.module.ShippingModuleConfig';
                        JSOpen({
    ...
    var paymethodSubmit = function (button) {
        ...
        var store = Ext.data.StoreManager.lookup("ShippingMethod");
        ...
        store.sync({
            ...
            failure: function (data, mata, succ) {
                ...
                /*              //没有下面这个statusText层次，且我在rest中做了统一的报错处理
