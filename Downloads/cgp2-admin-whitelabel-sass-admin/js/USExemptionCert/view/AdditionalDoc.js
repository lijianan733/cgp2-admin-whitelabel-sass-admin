Ext.define('CGP.USExemptionCert.view.AdditionalDoc', {
    ...
    initComponent: function () {
        ...
        me.title = i18n.getKey('其它附件');

        me.addField();
        me.callParent(arguments);

        me.show();
        if (Ext.isEmpty(me.additionalDocs)) {
            me.close();
            Ext.Msg.alert(i18n.getKey('prompt'), '没有其它附件！');
        }
        if (me.additionalDocs.length == 0) {
            me.close();
            Ext.Msg.alert(i18n.getKey('prompt'), '没有其它附件！');
        }
    ...
    addField: function () {
        ...
        me.additionalDocs ? me.additionalDocs.forEach(function (photo, index) {
            var imageUrl = imageServer + photo.url.split('/').pop();
            if (Ext.isEmpty(imageUrl)) {
                //图片名存放的属性不同
                imageUrl = imageServer + (photo.name || photo.originalFileName);
            }
            var previewComp;
            var imageItem;
                    ...
                    listeners: {
                        afterrender: function (view) {
                            Ext.create('Ext.ux.window.ImageViewer', {
                                ...
                                winConfig: {
                                    title: `${i18n.getKey('check')} < ${photo.originalFileName} > 预览图`
                                },
                                viewerConfig: null,
                            });
                        }
            ...
            items.push(item);
        }) : Ext.Msg.alert(i18n.getKey('prompt'), i18n.getKey('没有其它附件!'))
