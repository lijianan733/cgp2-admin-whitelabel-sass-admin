Ext.define("CGP.orderstatusmultilingual.OrderStatusMultilingual", {
    extend: 'Ext.tab.Panel',
    initComponent: function () {
        var me = this;
        var language = Ext.create('CGP.common.store.Language');
        var idArr = me.idArr;
        var frontendNameArr = me.frontendNameArr;
        //var nameArr = me.nameArr;
        ...
        language.load({callback: function (records, options, success) {
            Ext.Array.each(records, function (item) {
                ...
                resource.load({callback: function (records, options, success) {
                    Ext.Array.each(idArr, function (itemId, index) {
                        var key = me.multilingualKey + '-' + itemId + '-statusName';
                        var record = resource.findRecord('name', key);

                        var field = {
                            xtype: 'textfield',
                            itemId: itemId + '-statusName',
                            labelWidth: 200,
                            labelAlign: 'right',
                            //labelAlign: 'right',
                            fieldLabel: i18n.getKey('name') + '(' + itemId + '-'+frontendNameArr[index]+')',
                            locale: languageCode,
                            name: me.multilingualKey + '-' + itemId + '-statusName'
                        };
