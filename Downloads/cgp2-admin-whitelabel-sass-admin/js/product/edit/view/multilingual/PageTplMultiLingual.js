Ext.define("CGP.product.edit.view.multilingual.PageTplMultiLingual", {
        ...
        language.load({callback: function (records, options, success) {
            Ext.Array.each(records, function (item) {
                ...
                var resource = Ext.create('CGP.common.store.Resource', {
                    params: {
                        filter: '[{"name":"cultureCode","value":' + '"' + languageCode + '"' + ',"type":"string"},{"name":"name","value":' + '"%' + me.multilingualKey + '-' + me.pageTplId + '%"' + ',"type":"string"}]'
                    }
                });
                resource.load({callback: function (records, options, success) {
                    Ext.Array.each(attrArray, function (attribute, index) {
                        var key = me.multilingualKey + '-' + me.pageTplId + '-' + attribute;
                        ...
                        field = {
                            ...
                            locale: languageCode,
                            name: me.multilingualKey + '-' + me.pageTplId + '-' + attribute
                        };
                        if (record) {
