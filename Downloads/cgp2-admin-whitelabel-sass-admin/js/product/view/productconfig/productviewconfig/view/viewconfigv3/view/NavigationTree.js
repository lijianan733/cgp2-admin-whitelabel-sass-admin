Ext.syncRequire([]);
Ext.define("CGP.product.view.productconfig.productviewconfig.view.viewconfigv3.view.NavigationTree", {
    ...
    getValue: function () {
        ...
        result.clazz = 'com.qpp.cgp.domain.product.config.view.builder.dto.v3.OrdinaryBuilderViewConfigDTO';
        ...
        var defaultThemes = Ext.getCmp('defaultThemes');
        result.defaultThemes = defaultThemes.getValue() || null;
        return result;
    },
    ...
    initComponent: function () {
        ...
        var store = Ext.create('CGP.product.view.productconfig.productviewconfig.view.viewconfigv3.store.NavigationStore', {
                    ...
                    if (editViewConfigs) {
                        for (var i = 0; i < me.recordData.editViewConfigs.length; i++) {
                            ...
                            if (item.get('id') == editViewConfig.navItemId) {
                                //后来又加了个Id属性
                                Ext.isEmpty(editViewConfig.id) ? (editViewConfig.id = JSGetCommonKey()) : null;
            ...
            items: [
                {
                    xtype: 'combo',
                    editable: false,
                    fieldLabel: i18n.getKey('defaultTheme'),
                    itemId: 'defaultThemes',
                    name: 'defaultThemes',
                    id: 'defaultThemes',
                    ...
                    colspan: 4,
                    width: 350,
                    value: me.recordData ? me.recordData.defaultThemes : null,
                    store: Ext.create('Ext.data.Store', {
                        fields: ['display', "value"],
                        data: [
                            {
                                display: 'bgm', value: 'bgm'
                            },
                            {
                                display: 'ps', value: 'ps'
                            },
                            {
                                display: 'qpmn', value: 'qpmn'
                            }
                        ]
                    }),
                },
                ...
                {
                    xtype: 'button',
                    text: i18n.getKey('component') + i18n.getKey('manager'),
                    iconCls: 'icon_config',
                    flex: 1,
                    handler: function (btn) {
                        var componentArr = window.componentArr;
                        var navigationTree = Ext.getCmp('navigationTree');
                        var centerPanel = navigationTree.ownerCt.getComponent('centerBuilderViewConfigPanel');
                        if (centerPanel.record) {
                            if (centerPanel.isValid()) {

                            } else {
                                Ext.Msg.alert(i18n.getKey('prompt'), i18n.getKey('右侧组件配置必须完备'));
                                //直接返回null
                                return null;
                            }
                        }
                        var currentData = navigationTree.ownerCt.getCurrentValue();
                        if (currentData) {
                            var nextWin = Ext.create('CGP.product.view.productconfig.productviewconfig.view.viewconfigv3.view.managecomponent.ManageComponentInstanceWin', {
                                ...
                                componentInstanceArr: componentArr,
                                treePanelCfg: {},
                                allowCreate: true,
                                bbarCfg: {
                                    hidden: true
                ...
                },
                {
                    ...
                    handler: function (button) {
                        var treepanel = button.ownerCt.ownerCt;
                        var centerPanel = treepanel.ownerCt.getComponent('centerBuilderViewConfigPanel');
                        if (centerPanel.items.items[0].items.length > 0) {
                            //右边有详情信息了
                            var tbar = centerPanel.getDockedItems('toolbar[dock="top"]')[0];
                            var saveBtn = tbar.getComponent('saveBtn');
                            saveBtn.handler(saveBtn);
                        } else {
                            var result = treepanel.getValue();
                            var controller = Ext.create('CGP.product.view.productconfig.productviewconfig.view.viewconfigv3.controller.Controller');
                            controller.saveBuilderViewConfigDTO(treepanel.ownerCt, result, null);
                        }
                    }
                }
            ]
        ...
        me.listeners = {
            beforeselect: function (rowModel, record, index) {
                ...
                if (deselectRecord) {
                    if (
                        deselectRecord.get('clazz') == 'com.qpp.cgp.domain.product.config.view.navigation.dto.v3.FixedNavItemDTO' ||
                        deselectRecord.get('clazz') == 'com.qpp.cgp.domain.product.config.view.navigation.dto.v3.DynamicNavItemDTO'
                    ) {
            ...
            select: function (rowModel, record) {
                ...
                if (clazz == 'com.qpp.cgp.domain.product.config.view.navigation.dto.v3.FixedNavItemDTO') {
