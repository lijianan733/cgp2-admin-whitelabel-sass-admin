...
Ext.define("CGP.product.view.productconfig.productdesignconfig.view.salematerialbommvtmanage.view.BomTree", {
    ...
    initComponent: function () {
        ...
        var name = me.materialName;
        var materialId = me.getQueryString('materialId');
        var type = me.materialType;
        var store = Ext.create('CGP.product.view.productconfig.productdesignconfig.view.salematerialbommvtmanage.store.BomTree', {
            params: {
                ...
                handler: function (btn) {
                    ...
                    setTimeout(function () {
                        var win = Ext.create('CGP.common.commoncomp.CheckMaterialBomPictureWindow', {
                            imgSeverUrl: adminPath + 'api/productMaterialViewTypeDiagrams/' + me.productConfigDesignId,
                        });
        ...
        me.columns = [
            {
                ...
                renderer: function (value, metadata, record) {
                    ...
                    if (Ext.isEmpty(record.get('type')) && !record.isRoot()) {
                        ...
                        var icon = '';
                        if (record.get('isContainProductMaterialViewType')) {
                            icon += ' <img  style="width: 16px;height: 16px;vertical-align: middle;" src=' + path + 'ClientLibs/extjs/resources/themes/images/shared/32_32/accept.png>';
                        } else {
                            icon += ' <img  style="width: 16px;height: 16px;vertical-align: middle;" src=' + path + 'ClientLibs/extjs/resources/themes/images/shared/32_32/cog_add.png>';
                        }
                        return record.get("name") + '<' + id + '>'+icon;
                    }
                }
            }
        ];
        me.listeners = {
            ...
            beforeload: function (sto, operation, e) {
                ...
                if (Ext.isEmpty(type) && !operation.node.isRoot()) {
                    ...
                    sto.proxy.url = adminPath + 'api/productConfigDesigns/'+me.productConfigDesignId+'/bomTree/{id}/children?type=bomitem&materialId=' + realId;
                    delete sto.proxy.extraParams;
                } else {
                    /*sto.proxy.extraParams = {
                     type: 'material'
                     };*/
                    sto.proxy.url = adminPath + 'api/productConfigDesigns/'+me.productConfigDesignId+'/bomTree/{id}/children?type=material';
                }
            },
