Ext.define('CGP.product.view.productconfig.productdesignconfig.view.productmaterialmappings.controller.Controller', {
    editMaterialMapping: function (data, store, editOrNew, productConfigDesignId, productBomConfigId) {
        var me = this;
        Ext.create('CGP.product.view.productconfig.productdesignconfig.view.productmaterialmappings.edit', {
            store: store,
            data: data,
    ...
    deleteMaterialMapping: function (materialMappingId, store) {
        ...
        function callback(id) {
            if (id === 'yes') {
                Ext.Ajax.request({
                    url: adminPath + 'api/materialMappings/' + materialMappingId,
                    method: 'DELETE',
    ...
    updataMaterialMapping: function (data, store, win) {
        Ext.Ajax.request({
            url: adminPath + 'api/materialMappings/' + data._id,
            method: 'PUT',
            jsonData: data,
            headers: {Authorization: 'Bearer ' + Ext.util.Cookies.get('token')},
