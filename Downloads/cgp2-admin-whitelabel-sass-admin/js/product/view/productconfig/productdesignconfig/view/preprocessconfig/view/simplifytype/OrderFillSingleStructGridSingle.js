 ...
 *OrderFillSingleStructGridSinglePCPreprocessConfig
 */
...
Ext.define('CGP.product.view.productconfig.productdesignconfig.view.preprocessconfig.view.simplifytype.OrderFillSingleStructGridSingle', {
    ...
    initComponent: function () {
        ...
        me.items = [
            ...
            //     name: 'titleNumber',
            //     xtype: 'numberfield',
            //     fieldLabel: i18n.getKey('titleNumber'),
            //     itemId: 'titleNumber',
            //     allowBlank: false,
            //     hideTrigger: true
            // },
    ...
    getValue: function () {
        ...
        var items = me.items.items,
            result = {clazz: 'com.qpp.cgp.domain.preprocess.config.tile.OrderFillSingleStructGridSinglePCPreprocessConfig'};
        if (me.rendered == true) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.name) {
                    if (item.diySetValue) {
                        ...
                        Ext.Array.contains(['mvtgridfield', 'selectormapping'], item.xtype) ? item.setSubmitValue(data[item.name]) : item.setValue(data[item.name])
                    }
                } else {
                    item.setValue(data);
                }
