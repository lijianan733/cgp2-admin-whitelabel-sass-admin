Ext.define("CGP.attribute.view.multilingual.OptionMultilingual", {
    extend: 'Ext.tab.Panel',
    initComponent: function () {
        var me = this;
        var language = Ext.create('CGP.common.store.Language');
        var optionArr = [];
        if(!Ext.isEmpty(me.optionArr)){
            optionArr = me.optionArr.split(',');
        }
        var optionNameArr = [];
        if(!Ext.isEmpty(me.optionNameArr)){
            optionNameArr = me.optionNameArr.split(',');
        }
        ...
        language.load({callback: function (records, options, success) {
            Ext.Array.each(records, function (item) {
                ...
                resource.load({callback: function (records, options, success) {
                    Ext.Array.each(optionArr, function (optionId, index) {
                        var key = me.multilingualKey + '-' + optionId + '-name';
                        var record = resource.findRecord('name', key);

                        var field = {
                            xtype: 'textfield',
                            itemId: optionId + '-name',
                            labelWidth: 200,
                            //labelAlign: 'right',
                            fieldLabel: i18n.getKey('name') + '(' + optionId + '-'+optionNameArr[index]+')',
                            locale: languageCode,
                            name: me.multilingualKey + '-' + optionId + '-name'
                        };
