Ext.define('CGP.orderdetails.view.lineitem.Photo', {
    ...
    addField: function () {
        ...
        me.photos ? me.photos.forEach(function (photo, index) {
            ...
            if (/^blob:/.test(imageUrl)) {
                //当用blob开头的格式的url时，进行特殊处理，用imageServer+fileName进行拼接图片地址，然后按照图片进行展示处理
                imageUrl = imageServer + (photo.name || photo.fileName);
                photo.src = imageUrl + '/150/150';//设置预览图大小

            }
            if (Ext.Array.contains(['bmp', 'jpg', 'png', 'jpeg', 'svg'], imageUrl.split('.').pop())) {
                previewComp = {
